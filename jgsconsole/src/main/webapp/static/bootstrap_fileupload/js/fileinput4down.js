...
//            '    <div class="close fileinput-remove">&times;</div>\n' +
        ...
        tActionDelete = '<button type="button" class="kv-file-remove {removeClass}" ' +
            'title="{removeTitle}"{dataUrl}{dataKey} delUrl="{delUrls}">{removeIcon}</button>\n',
        tActionUpload = '<button type="button" class="kv-file-upload {uploadClass}" title="{uploadTitle}">' +
            '   {uploadIcon}\n</button>\n',
        tGeneric = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}">\n' +
            '   {content}\n' +
            '   {footer}\n' +
            '</div>\n',
        tHtml = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}">\n' +
            '    <object data="{data}" type="{type}" width="{width}" height="{height}">\n' +
            '       ' + DEFAULT_PREVIEW + '\n' +
            '    </object>\n' +
            '   {footer}\n' +
            '</div>',
        tImage = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}">\n' +
            '   <img src="{data}" class="file-preview-image" title="{caption}" alt="{caption}" ' + STYLE_SETTING + '>\n' +
            '   {footer}\n' +
            '</div>\n',
        tText = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}">\n' +
            '   <div class="file-preview-text" title="{caption}" ' + STYLE_SETTING + '>\n' +
            '       {data}\n' +
            '   </div>\n' +
            '   {footer}\n' +
            '</div>',
        tVideo = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}"' +
            ...
            '   {footer}\n' +
            '</div>\n',
        tAudio = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}"' +
            ' title="{caption}" ' + STYLE_SETTING + '>\n' +
            '   <audio controls>\n' +
            '       <source src="' + '{data}' + '" type="{type}">\n' +
            '       ' + DEFAULT_PREVIEW + '\n' +
            ...
            '   {footer}\n' +
            '</div>',
        tFlash = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}"' +
            ' title="{caption}" ' + STYLE_SETTING + '>\n' +
            ...
            '   {footer}\n' +
            '</div>\n',
        tObject = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}"' +
            ' title="{caption}" ' + STYLE_SETTING + '>\n' +
            '   <object data="{data}" type="{type}" width="{width}" height="{height}">\n' +
            ...
            '   {footer}\n' +
            '</div>',
        tOther = '<div class="file-preview-frame{frameClass}" id="{previewId}" dfindex="{dfindex}" data-fileindex="{fileindex}"' +
            ' title="{caption}" ' + STYLE_SETTING + '>\n' +
            '   ' + DEFAULT_PREVIEW + '\n' +
            '   {footer}\n' +
            '</div>',
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                setTimeout(function () {
                    if (data.error === undefined) {
                        ...
                        $btnUpload.hide();
                        $btnDelete.hide();
                        if(outData.response.resultFlag=='success'){
                        	parent.reload;
            				parent.$.modalDialog.openner.datagrid('insertRow',{
            					index : 0,
            					row : outData.response.resultMap
            				});
            				parent.$.modalDialog.handler.dialog('close');
            				swalAlert('上传成功','资料已上传','success',2000);
                        }else{
                        	swalAlert('上传失败','资料上传失败，请稍后重试！','error',2000);
                        }
                        ...
                        self.showUploadError(outData.files[i].name+'：'+outData.response.result, params);
                    } else {
            ...
            if (chkTypes && chkMimes) {
                if (cat === 'text') {
                    ...
                    content = tmplt.repl('{previewId}', previewId).repl('{caption}', caption)
                        ...
                        .repl('{footer}', footer).repl('{fileindex}', ind).repl('{dfindex}',ind) + modal;
                } else {
                    content = tmplt.repl('{previewId}', previewId).repl('{caption}', caption)
                        .repl('{frameClass}', '')
                        .repl('{type}', file.type).repl('{data}', data)
                        .repl('{width}', config.width).repl('{height}', config.height)
                        .repl('{footer}', footer).repl('{fileindex}', ind).repl('{dfindex}',ind);
                }
                self.$preview.append("\n" + content);
    ...
    $.fn.fileinput.locales.en = {
            ...
            dropZoneTitle: '拖拽文件到这里，或点击右下角的选择按钮&hellip;',
