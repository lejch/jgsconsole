...
var cpyTypeTransList = ['','私营','集体','联营','三资','国有','其他'];
var cpyBelongTransList = ['','农林渔牧业','商贸','房地产','制造业','服务','其他行业'];
	...
	$('#searchbox').searchbox({
	    ...
	    searcher:function(value,name){
		    	jspReqControl(function(){
		    		...
		    		$grid=$dg.datagrid({
					url: appPath+"/cpymg/getCpyRegBySearch",
	...
	$grid=$dg.datagrid({
		...
		columns : [[
		            {field:'HY_ID',title:'会员号',width:30,align:'left',halign:'center',formatter:function(value,row,index){
		            	if(row.FLAG=='0'){
		            		return '<span class="label label-success" style="background-color:#0099FF !important;margin-right:10px;margin-left:5px;">企业</span>'+value
		            	}else{
		            		return '';
		            	}
		            }},
		            {field:'CPY_NAME_CN',title:'企业名称(中)',width:30,align:'center'},
		            {field:'CPY_NAME_EN',title:'企业名称(英)',width:30,align:'center'},
		            {field:'CPY_ADDR',title:'单位地址',width:30,align:'center'},
		            ...
		            {field:'SQZT',title:'申请状态',width:20,align:'center',formatter:function(value,row,index){
		            	if(row.FLAG=='0'){
		            		...
		            		'<i style="margin-left:8px;" id="'+row.ID+'btg" sqname="'+row.USERALIAS+'" firstCrt="'+row.CREATETIME+'" hyerr="'+row.QYSH_ERR_REASON+'" onclick="showHerrasn(this)" class="glyphicon glyphicon-question-sign zytzflag" onmouseleave="delBtnTips()" onmouseenter="initBtnTips5(this)"></i>';
		            	}
		            }},
		            {field:'FLAG',title:'操作',width:20,align:'center',formatter:function(value,row,index){
		            	if(value=='1'){
		            		...
		            		+'" onclick="passReg(this)" onmouseleave="delBtnTips()" onmouseenter="initBtnTips(this)">'+
				            		...
				            		+'" onclick="refuseReg(this)" onmouseleave="delBtnTips()" onmouseenter="initBtnTips3(this)">'+
		...
		onDblClickRow:function(rowIndex, rowData){
			jspReqControl(function(){
				parent.$.modalDialog({
					...
					width : '650',
					height : '700',
					href : appPath+"/views/wlcmConsole/org_company/cpyRegStepEditDlg.jsp",
					onLoad:function(){
						rowData.CPY_TYPE_TRANS = cpyTypeTransList[parseInt(rowData.CPY_TYPE)];
						rowData.CPY_BELONG_TRANS = cpyBelongTransList[parseInt(rowData.CPY_BELONG)];
		...
		jspReqControl(function(){
			jspReqControl(function(){
				layer.open({title:false,move:false,id:'synotications',type:1,resize:false,area:['1030px',(height)+'px'],shade:[0.1,'#fff'],shadeClose:true,closeBtn:0,zIndex:1,
					success:function(layero, index){
						...
						$('#synoticationsdvt').datagrid({
							...
							columns : [[
							            ...
							            {field:'CPY_NAME_CN',title:'企业名称(中)',width:150,align:'center'},
							            {field:'CPY_NAME_EN',title:'企业名称(英)',width:150,align:'center'},
							            {field:'CPY_CREATE_TIME',title:'成立时间',width:80,align:'center'},
							            {field:'CPY_GSZCH',title:'工商注册号',width:150,align:'center'},
							            {field:'CPY_TYPE',title:'企业性质',width:60,align:'center',formatter:function(value,row,index){
							            	return cpyTypeTransList[parseInt(value)];
							            }},
							            {field:'CPY_TYPE_OTHER',title:'企业性质补充',width:200,align:'center'},
							            {field:'CPY_BELONG',title:'所属行业',width:60,align:'center',formatter:function(value,row,index){
							            	return cpyBelongTransList[parseInt(value)];
							            }},
							            {field:'CPY_BELONG_OTHER',title:'所属行业补充',width:200,align:'center'},
							            {field:'CPY_ADDR',title:'单位地址',width:200,align:'center'},
							            {field:'CPY_UB',title:'邮编',width:80,align:'center'},
							            {field:'FR_NAME',title:'法人姓名',width:80,align:'center'},
							            {field:'FR_ZW',title:'法人职务',width:80,align:'center'},
							            {field:'FR_BGDH',title:'法人办公电话',width:150,align:'center'},
							            {field:'FR_PHONE',title:'法人手机',width:150,align:'center'},
							            {field:'LXR_NAME',title:'联系人姓名',width:80,align:'center'},
							            {field:'LXR_ZW',title:'联系人职务',width:100,align:'center'},
							            {field:'LXR_BGDH',title:'联系人办公电话',width:150,align:'center'},
							            {field:'LXR_PHONE',title:'联系人手机',width:150,align:'center'},
							            {field:'LXR_EMAIL',title:'联系人电子邮件',width:100,align:'center'},
							            {field:'JL',title:'业务范围',width:300,align:'center'}
							            ]]
						});
						...
						document.getElementById('grcxhy').onclick=function(){
							...
							$('#synoticationsdvt').datagrid({url: appPath+"/cpymg/getCpyHz",
						...
						document.getElementById('dcgrhy').onclick=function(){
							if(($('#synoticationsdvt').datagrid('getData')).rows.length>0){
								initExportWithThreeParam(appPath+"/cpymg/exportAllGroupReg",
					...
					'<td><div id="grcxhy"><a>查询企业会员</a></div></td>'+
					'<td><div id="dcgrhy"><a>导出企业会员</a></div></td>'+
					'</tr></table>'+
...
function getBkReg(obj){
	jspReqControl(function(){
		$.ajax({
			   type: "POST",
			   async: false,
			   url: appPath+"/cpymg/getBkGroupReg",
		       ...
		       success: function(data){
		    	   if(data){
		    		   $dg.datagrid('reload');
		    		   swalAlert("撤销成功","该企业会员审核通过状态，已被撤销!","success",2000);
		    	   }else{
					...
					if($('#disReason').validatebox('isValid')){
						if(DisZhBtnIsNotClick){
							...
							$.ajax({type:"POST",async:false,timeout:120*1000,dataType:"json",
								...
								url:appPath+"/cpymg/passGroupReg",
								success: function(data){
									if(data){
										layer.closeAll();
										if(data){
								    		   $dg.datagrid('reload');
								    		   swalAlert("审核成功","该企业会员已通过审核!","success",2000);
								    		   if($(obj).attr('hsont')=='1'){
					...
					if($('#disReason').textbox('isValid')){
						if(DisZhBtnIsNotClick){
							...
							$.ajax({type:"POST",async:false,timeout:120*1000,dataType:"json",
								...
								url:appPath+"/cpymg/refuseGroupReg",
								success: function(data){
									if(data){
										layer.closeAll();
										if(data){
								    		   $dg.datagrid('reload');
								    		   swalAlert("已拒绝申请","该企业会员申请,已被拒绝审核!","success",2000);
...
function initBtnTips2(obj){event.stopPropagation();layer.tips('导出企业申请','#'+$(obj).attr('id'),{tips:1});}
