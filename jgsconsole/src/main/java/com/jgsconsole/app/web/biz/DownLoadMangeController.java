...
import com.jgsconsole.app.service.biz.DownLoadMangeService;

@Controller
@RequestMapping("/dlm")
public class DownLoadMangeController {

	@Autowired
	public DownLoadMangeService downLoadMangeService;
	...
	public String fileUpload(@RequestParam("file") MultipartFile file,HttpServletRequest request,HttpServletResponse response) {
		...
		if (!file.isEmpty()) {
			try {
				...
				map.put("FILE_LOC","download/"+newFileName+"."+FILE_TYPE);
				map.put("CREATOR",user.getUseralias());
				System.out.println(map);
				downLoadMangeService.insertUploadFile(map);
	...
	public Object getDownloadList(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		data = downLoadMangeService.getDownloadList(rowbounds);
	...
	public Object delUpFile(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if(hasPdf>=0&&hasPdf<18){
			...
			pdffilepath = pdffilepath.replaceAll("download", "previewpdf");
		}
		map.put("ID", ID);

		downLoadMangeService.delUpFile(map,request.getSession().getServletContext().getRealPath("/")+UPURI,hasPdf2del,request.getSession().getServletContext().getRealPath("/")+pdffilepath);
